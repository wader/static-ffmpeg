...
# bump: whispercpp /WHISPERCPP_VERSION=([\d.]+)/ https://github.com/ggml-org/whisper.cpp|^1
# bump: whispercpp after ./hashupdate Dockerfile WHISPERCPP $LATEST
# bump: whispercpp link "Release notes" https://github.com/ggml-org/whisper.cpp/releases/tag/v$CURRENT
ARG WHISPERCPP_VERSION=1.8.3
ARG WHISPERCPP_URL="https://github.com/ggml-org/whisper.cpp/archive/refs/tags/v$WHISPERCPP_VERSION.tar.gz"
ARG WHISPERCPP_SHA256=870ba21409cdf66697dc4db15ebdb13bc67037d76c7cc63756c81471d8f1731a
RUN \
  wget $WGET_OPTS -O whispercpp.tar.xz "$WHISPERCPP_URL" && \
  echo "$WHISPERCPP_SHA256  whispercpp.tar.xz" | sha256sum --status -c - && \
  tar $TAR_OPTS whispercpp.tar.xz && cd whisper.cpp-* && \
  cmake -B build -DBUILD_SHARED_LIBS=OFF && \
  cmake --build build -j --config Release --target install
RUN sed -i 's/-lwhisper/-lwhisper -lstdc++/' /usr/local/lib/pkgconfig/whisper.pc
